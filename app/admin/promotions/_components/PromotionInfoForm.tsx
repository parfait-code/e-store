// app/admin/promotions/_components/PromotionInfoForm.tsx
"use client";
...
export function PromotionInfoForm({
  initialPromotion,
  onSuccess,
}: {
  ...
  const isEditing = Boolean(initialPromotion);
  const [slugTouched, setSlugTouched] = useState(isEditing);
  const [form, setForm] = useState<PromotionFormInput>({
    name: initialPromotion?.name ?? "",
    slug: initialPromotion?.slug ?? "",
    description: initialPromotion?.description ?? "",
    startDate: initialPromotion?.startDate.slice(0, 16) ?? "",
    endDate: initialPromotion?.endDate.slice(0, 16) ?? "",
    isActive: initialPromotion?.isActive ?? true,
  });
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      const result = isEditing
        ? await apiClient.put<Promotion>(
            `/promotions/${initialPromotion!.id}`,
            payload,
          )
        : await apiClient.post<Promotion>("/promotions", payload);
      onSuccess(result);
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          />
          {fieldError("endDate") && (
            <p className="mt-1 text-xs text-red-600">{fieldError("endDate")}</p>
          )}
        </div>
      </div>
